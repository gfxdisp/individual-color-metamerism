...
for indice = 1:size(valid_indices,1)
    if (valid_indices(indice,1) == 0)
        ...
        scatter(E_mean_flat(indice), MLE_std_flat(indice), 'MarkerFaceColor', point_color_flat(indice, :), 'MarkerEdgeColor', point_color_flat(indice, :));
        text(E_mean_flat(indice), MLE_std_flat(indice) - 0.01, sprintf("{%d, %d}", display_patterns(point_display_pattern_flat(indice),1), ...
...
correlation_coefficient_Pearson = corr(E_mean_flat_valid, MLE_std_flat_valid, 'type','Pearson');
correlation_coefficient_Kendall = corr(E_mean_flat_valid, MLE_std_flat_valid, 'type','Kendall');
correlation_coefficient_Spearman = corr(E_mean_flat_valid, MLE_std_flat_valid, 'type','Spearman');
...
ylabel('MLE STD');
title('MLE STD w and 1000 CMFs deltaE 2000 Mean points');